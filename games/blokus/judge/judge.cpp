...
struct program
{
    ...
    {
        this -> file = strdup(s);
        //socketpair(PF_LOCAL, SOCK_STREAM, 0, this->fd);
        this -> score = 0;
    }
};
...
struct board
{
    ...
    void start()
    {
        ...
        fclose(data);
        sprintf(buf,"%d %d %d %d %d %d %d",0,0,0,0,0,0,0);
        time_t tim;
        time(&tim);
    ...
    bool isLawful(int ID,int x,int y,int block_id,int l,int v,int d)
    {
        ...
        for (int i = 0; i < block[block_id].num; i++)
        {
            ...
            if (outIt(x+addx,y+addy)) return false;
            if (this -> color[x+addx][y+addy]) return false;
            if (ID == this -> color[x+addx-1][y+addy]) return false;
            if (ID == this -> color[x+addx][y+addy+1]) return false;
            if (ID == this -> color[x+addx+1][y+addy]) return false;
            if (ID == this -> color[x+addx][y+addy-1]) return false;
            if (ID == this -> color[x+addx-1][y+addy-1]) flag = true;
            if (ID == this -> color[x+addx-1][y+addy+1]) flag = true;
            if (ID == this -> color[x+addx+1][y+addy-1]) flag = true;
            if (ID == this -> color[x+addx+1][y+addy+1]) flag = true;
            if (this -> first[ID])
            {
                if (ID == 1 && x+addx == 1 && y+addy == 1)this -> first[ID] = false;
                if (ID == 2 && x+addx == 1 && y+addy == 20)this -> first[ID] = false;
                if (ID == 3 && x+addx == 20 && y+addy == 20)this -> first[ID] = false;
                if (ID == 4 && x+addx == 20 && y+addy == 1)this -> first[ID] = false;
                if (!this -> first[ID]) flag = true;
            }
        }
        return flag;
    }
    bool judge_pass(char* s)
    {
        int ID,x,y,block_id,l,v,d;
        sscanf(s,"%d%d%d%d%d%d%d",&ID,&x,&y,&block_id,&l,&v,&d);
        return isLawful(ID,x,y,block_id,l,v,d);
    }
    void addGamebox(char* s)
    {
        int ID,x,y,block_id,l,v,d;
        sscanf(s,"%d%d%d%d%d%d%d",&ID,&x,&y,&block_id,&l,&v,&d);
        if (block_id)   this -> used[ID][block_id] = true;
        if (!block_id)   this -> pass_num++;
        else this -> pass_num = 0;
        if (block_id)
            for (int i = 0; i < block[block_id].num; i++)
            {
                int addx = block[block_id].box[i].x;
        ...
        if (this -> pass_num == 4)
        {
            ...
            for (int i = 1; i <= 4; i++)
                for (int j = 1; j <= 21; j++)
                    if (this -> used[i][j]) player_score[i] += block[j].num;
            if (player_score[1] + player_score[3] <= player_score[2] + player_score[4]) this -> victor = 2;
            else this -> victor = 1;
        }
    }
    void print(){
...
int Judge::after_read(string line)
{
    ...
    char * buff = strdup(line.c_str());
    strcpy(buf,buff);
    if (!Game -> judge_pass(buf))
    {
